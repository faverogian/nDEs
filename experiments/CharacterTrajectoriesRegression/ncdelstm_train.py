...
from src.data.transforms import fill_forward
...
def logger(train_stats, test_loss):
    with open(f'./{HP["log_dir"]}/log_ncdelstm.txt', 'w') as f:
...
def plot_trajectory(pred_y, batch_y):
    ...
    # Save the plot
    plt.savefig(f'./{HP["log_dir"]}/trajectory_cdelstm.png')
...
def main(device):
    ...
    # Save model
    torch.save(best_model.state_dict(), f'./{HP["log_dir"]}/ncdelstm_model.pth')
